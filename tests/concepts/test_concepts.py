...
def test_not_backend() -> None:
    ...
    _: concepts.Backend = NotBackend0() # type: ignore[assignment]

    class NotBackend1:
        def run(self: Self, program: QuantumProgram) -> None:
            return
    _test_backend(NotBackend1()) # type: ignore[arg-type]
    _: concepts.Backend = NotBackend1() # type: ignore[assignment]
