...
def test_to_dataframe_with_empty_solutions_list() -> None:
    result = analysis.to_dataframe([])
    check.is_true(result.empty)
    check.equal(list(result.columns), ["labels", "bitstrings", "costs", "counts", "probs"])
...
def test_filter_by_percentage(
    top_percent: float, order: Literal["ascending", "descending"], expected_costs: list[int]
) -> None:
...
def test_filter_by_percentage_keeps_all_rows_when_group_never_reaches_threshold() -> None:
    ...
    result = analysis._filter_by_percentage(df, top_percent=0.9)
    assert sorted(result["costs"].tolist()) == [1, 2, 3]


def test_filter_by_percentage_missing_column_raises() -> None:
    with pytest.raises(ValueError):
        analysis._filter_by_percentage(_df(), column="gaps")


def test_filter_by_percentage_invalid_top_percent_raises() -> None:
    with pytest.raises(ValueError):
        analysis._filter_by_percentage(_df(), top_percent=0.0)
    with pytest.raises(ValueError):
        analysis._filter_by_percentage(_df(), top_percent=1.5)
