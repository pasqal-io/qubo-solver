...
from qubosolver import Instance, solving, Solver, matrix, bitstrings, LocalEmulator
...
def test_classical_all_positive_trivial() -> None:
    ...
    config = solving.Config(solving=solving.classical.Config())
...
def test_quantum_all_negative_trivial(local_backend: LocalEmulator) -> None:
    ...
    """
    config = solving.Config(solving=solving.quantum.Config(backend=local_backend))
