...
class MeanEnergyHeuristicDriveShaper(_BaseDriveShaper):
    r"""Heuristic drive shaper based on the mean local physical energy.

    For each qubit, the local physical energy scale is

        E_i = |delta_i(T)| + sum_{j != i} |V_ij|,

    where delta_i(T) is the final local detuning and V_ij is the
    physical interaction generated by the embedded register.
    ...
    def generate(
        ...
        """Generate a drive using the mean-energy heuristic schedule.

        Args:
            register: The physical register layout.

        Returns:
            The generated mean-energy drive and an empty solution.
        """
        device = self.config.device
        dmm = self.config.drive_shaping.dmm
        kappa = self.config.drive_shaping.heuristic_kappa
        ...
        return (
            mean_energy_heuristic.build_drive(
                self.instance,
                register,
                device=device,
                dmm=dmm,
