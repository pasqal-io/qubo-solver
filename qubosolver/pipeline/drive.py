...
class MeanEnergyHeuristicDriveShaper(BaseDriveShaper):
    r"""
    Heuristic drive shaper using the average local physical energy scale.

    For each qubit, the local physical energy scale is

        E_i = |delta_i(T)| + sum_{j != i} |V_ij|,

    where delta_i(T) is the final detuning effectively applied to
    qubit i and V_ij is the physical interaction generated by the
    embedded register.

    The maximum mixing strength is

        omega_max = kappa * mean_i(E_i).

    With DMM, the final local detunings are encoded as
    ...
    Without DMM, all qubits receive the same final global detuning
    ...
    def generate(
        ...
        # Heuristic scaling coefficient.
        kappa = self.config.drive_shaping.heuristic_kappa

        # Hardware bounds.
        specs = self.device.specs
        max_seq_duration: float = (
            specs["max_duration"] or 1000.0
        )
        ...
        max_amplitude = (
            specs["max_amplitude"] or 1.0
        )
        max_abs_detuning = (
            specs["max_abs_detuning"] or 9.0
        )

        det_amp_ratio = (
            max_amplitude / max_abs_detuning
        )

        if kappa < det_amp_ratio:
            warnings.warn(
                "heuristic_kappa is too small "
                f"({kappa}); a Qoolqit CompilationError "
                "may occur. "
        ...
        # Target local final detunings used to construct
        # the detuning and DMM waveforms.
        d = (
            -0.5 * torch.diag(Q)
        ).detach().cpu().numpy()
        ...
        if use_dmm:
            # Final global detuning is the largest target
            # local detuning.
            delta_g_T = d_max

            # DMM locally lowers the global detuning.
            spread = max(
                0.0,
                d_max - d_min,
            )
        ...
        # ---------------------------------------------------------
        # New physical-energy definition of omega_max
        # ---------------------------------------------------------

        # Final detuning effectively applied to every qubit:
        #
        # delta_i(T) =
        ...
        # Actual dimensionless physical interactions
        # generated by the embedded register:
        #
        # V_ij = 1 / r_ij**6.
        ...
        qubit_index = {
            qubit_id: index
            for index, qubit_id
            in enumerate(register.qubits_ids)
        }
        ...
        # ---------------------------------------------------------
        # End of the new omega_max calculation
        # ---------------------------------------------------------

        # Keep the initial detuning rule unchanged.
        delta_0 = -float(
            np.max(np.abs(d))
        )
        ...
        # Optional DMM weighted detuning.
        dmm = None
...
def get_drive_shaper(
    ...
    if config.drive_shaping.drive_shaping_method == DriveType.HEURISTIC:
        ...
        return MeanEnergyHeuristicDriveShaper(instance, config, backend)
    elif issubclass(
        config.drive_shaping.drive_shaping_method,
        BaseDriveShaper,
    ):
